...
# Getting started
Keep in mind that this application is not aimed at providing a template or reference of implementation. It is made by and for me specifically. The code is publicly available to serve as inspiration/proof of concept, and is not meant to be forked and run without modifications.
...
* `IGDB_USER_KEY` - Personal API key acquired from IGDB website
* `NEXT_PUBLIC_SENTRY_DSN` - Project DSN for the Sentry project

You can verify that you have the required env variables by running `yarn verify:env`. If any of the ones required in the script is missing, it will error out.
Inside `src/config.server.ts` you will find my encrypted public/private keys. Either replace them with another set of encrypted keys using your iv:secret or put them into your env and replace their reference here.
...
To run the application locally, run `yarn dev`. This will start both Next and Storybook. If you want to start any of them up individually, use `yarn dev:next` or `yarn dev:storybook`
...
Once build, the application can be run with `yarn start`. Storybook is put into `/public` and is a static build.
...
To make use of FaunaDB and Vercel to use all of the features implemented in this application, there is a indirect expectation that you know how to use both of these services.
If you do not know how to, there is plenty of documentation out there that will help you learn how to use these amazing tools. This project does not aim to explain any used services.
