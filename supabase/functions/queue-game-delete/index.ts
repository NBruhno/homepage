...
Deno.serve(async (req) => {
	try {
		...
		try {
			...
			`
			console.log(`Added delete request for game with ID ${id} to queue.`)
		} catch (error) {
			console.error(`Failed to add delete request for ID ${id} to queue`)
			throw error
		} finally {
			await db.end()
		}
		...
		return new Response(
			JSON.stringify({
				message: `Added delete request for game with ID ${id} to queue.`,
			}),
