	...
	describe('Action Creators', () => {
		describe('Get Stars (Async)', () => {
			...
			it('dispatches Request and Success Actions on OK requests', (done) => {
				...
				const expectedActions: StarsAction[] = [{ type: stars.GET_REQUEST }, { type: stars.GET_SUCCESS, payload: { count: githubResponse.stargazers_count } }]

				const store = mockStore({})

				store
					.dispatch(stars.getStars())
			...
			it('dispatches Failure on failed requests', (done) => {
				...
				const expectedActions: StarsAction[] = [{ type: stars.GET_REQUEST }, { type: stars.GET_FAILURE, payload: { message: errResponse } }]
