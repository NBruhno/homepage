...
const GameEvents: NextPage = () => {
	...
	return (
		<Page>
			...
			<PageContent maxWidth={1300} style={{ zIndex: 1, position: 'relative' }}>
				<div
					style={{
						display: 'flex',
						columnGap: '12px',
				...
				>
					<div>
						...
						<Subtitle style={{ display: 'flex', columnGap: '12px', justifyContent: 'center' }}>
							{event.startTime && (
								<span>
				...
				{videoId ? <Video id={videoId} name={`${event.name} live stream`} /> : <EventCover coverUrl={event.logo} />}

				<p>{event.description}</p>

				<ListGroup>
					<div style={{ rowGap: '12px', display: 'flex', flexDirection: 'column' }}>
