...
export default apiHandler({ validMethods: ['POST'], cacheStrategy: 'NoCache' }).post(async (req, res) => {
	...
	if (queueStrategy === 'amqp') {
			...
			const [createdGames, updatedGames, deletedGames] = await Promise.all([
				(async () => {
					...
					if (toCreate.length > 0) {
						const games = create(
							(
								await Promise.all(
									chunk(toCreate, 500).map(async (ids) =>
				...
				(async () => {
					...
					if (updateRequestExists.length > 0) {
						...
						const games = (
							await Promise.all(
								chunk(toUpdate, 500).map(async (ids) =>
									igdbFetcher<IgdbGame, false>('/games', res, {
										shouldReturnFirst: false,
										body: `${gameFields}; limit 500; where id = (${ids.join(',')});`,
										nickname: `outdated games, 0-500`,
						...
						const updateQueries = games.map((game) =>
							prisma.games.update({
								where: { id: game.id },
								data: game,
				...
				(async () => {
					...
					if (deleteRequestExists.length > 0) {
						...
						const deleteQueries = toDelete.map((id) =>
							prisma.games.delete({
								where: { id },
								select: { id: true },
							}),
		...
		const { createRequests, updateRequests, deleteRequests } = Object.entries(changeRequests).reduce<{
			createRequests: Array<number>
			...
			(
				...
				switch (type) {
					case 'create':
						return { ...requests, createRequests: [...requests.createRequests, id] }
					case 'update':
						return { ...requests, updateRequests: [...requests.updateRequests, id] }
					default:
						return { ...requests, deleteRequests: [...requests.deleteRequests, id] }
				}
			},
			...
			(async () => {
				if (createRequests.length > 0) {
					const games = create(
						(
							await Promise.all(
								chunk(createRequests, 500).map(async (ids) =>
									igdbFetcher<IgdbGame, false>('/games', res, {
										shouldReturnFirst: false,
										body: `${gameFields}; limit 500; where id = (${ids.join(',')});`,
										nickname: `outdated games, 0-500`,
