...
const { email, password, defaultPassword, username, accessCode } = createCredentials({
	label: 'change-password',
	shouldPrefixEmail: true,
})
...
describe('/api/users/{userId}/changePassword', () => {
	beforeAll(async () => {
		...
		const res = (await supertest(usersServer).post('/api/users').send({
			email,
			username,
		...
		if (res.status !== 200) {
			...
			await supertest(deleteServer).delete(`/api/users/${id}`).set('authorization', `Bearer ${accessToken}`)
			deleteServer.close()
	...
	test('POST › Not authenticated', async () => {
		expect.hasAssertions()
		const server = createTestServer(handler, { id })
		const res = await supertest(server).post(`/api/users/${id}/changePassword`).send({
			newPassword: password,
		})
