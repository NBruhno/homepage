...
const newsMapper = (news: SteamNews['appnews']['newsitems']) =>
	news.map(({ gid, title, url, contents, date, feedlabel: feedLabel, feedname: feedName }) => ({
		id: gid,
		url,
		title,
...
export default apiHandler({
	...
	const news = await monitorAsync(
		...
		const [steamCommunityNews, otherNews] = partition(
			steamNews.appnews.newsitems,
			({ feed_type: feedType, feedname }) => feedType === 1 || feedname === 'steam_community_announcements',
		)
