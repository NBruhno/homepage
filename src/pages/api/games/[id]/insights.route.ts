...
export default apiHandler({
	...
	const insights = await monitorAsync(
		() =>
			fetch(`https://vginsights.com/api/v1/game/${appId}`, {
				method: 'GET',
				headers: {
		...
		if (!response.ok) {
			throw ApiError.fromCode(response.status as unknown as keyof typeof statusCodes)
		}
	...
	if (insights.rating && insights.unitsSold) {
		const [history, stats] = await Promise.all([
			monitorAsync(
				() =>
					fetch(`https://steamcharts.com/app/${appId}/chart-data.json`, {
						method: 'GET',
					}),
				'http:steamcharts',
				'game player history',
			...
			monitorAsync(
				() =>
					fetch(`https://vginsights.com/api/v1/game/${insights.slug}/quick-stats`, {
						method: 'GET',
						headers: {
							authorization: `Bearer ${config.vgInsights.token}`,
						},
