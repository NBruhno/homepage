...
export const useGames = () => {
	...
	const { data, error } = useSWR(state.user?.isStateKnown ? ['/games', query] : null, (link, query) => fetcher(link, { body: query, accessToken: state.user?.accessToken, method: Method.Post }), { revalidateOnFocus: false })
...
export const useGame = (id: string) => {
	...
	const { state } = useStore()
	const { data, error } = useSWR((id && state.user?.isStateKnown) ? `/games/${id}` : null, (link) => fetcher(link, { accessToken: state.user?.accessToken }), { revalidateOnFocus: false })
