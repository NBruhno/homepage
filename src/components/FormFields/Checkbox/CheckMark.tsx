...
export const CheckMark = ({ isChecked, isDisabled, isFocusVisible, isHovered, ...rest }: Props) => (
	<div
		css={(theme) => ({
			...
			position: 'relative',
			boxShadow: isFocusVisible ? `0 0 0 2px ${theme.color.primary}` : 'none',
			border: '1px solid',
			borderRadius: '4px',
			width: '22px',
			height: '22px',
			...
			margin: 'auto',
			borderColor: (() => {
				if ((isHovered || isFocusVisible) && !isDisabled) return isChecked ? theme.color.primaryLighter : theme.color.primary
