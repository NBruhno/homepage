	...
	(req: NextApiRequest, res: NextApiResponse) => {
		...
		if (allowedMethods.includes(req.method ?? '')) {
			const apiError = ApiError.fromCodeWithCause(
				404,
				new Error(
					`Could not find any matching route with the given method. Received "${req.method ?? 'UNSPECIFIED'}" - "${req.url ?? 'Unknown URL'}"`,
				),
			)
			...
			const apiError = ApiError.fromCodeWithCause(
				405,
				new Error(
					`Method not allowed. Expected one of [${allowedMethods.join(', ')}] but received "${req.method ?? 'UNSPECIFIED'}"`,
				),
			)
			updateTransaction({ status: apiError.statusCode })
