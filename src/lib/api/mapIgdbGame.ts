import type { IgdbCompany, IgdbGame, IgdbMultiplayerMode, IgdbWebsite } from 'types'
		...
		return websites
			...
			.map(({ category, url }) => ({
				type: (() => {
					switch (category) {
						case IgdbWebsiteCategory.Android:
							return GameWebsiteType.GooglePlayStore
						case IgdbWebsiteCategory.Discord:
							return GameWebsiteType.Discord
						case IgdbWebsiteCategory.EpicGames:
							return GameWebsiteType.EpicGames
						case IgdbWebsiteCategory.GoG:
							return GameWebsiteType.GoG
						case IgdbWebsiteCategory.Ipad:
						case IgdbWebsiteCategory.Iphone:
							return GameWebsiteType.AppStore
						case IgdbWebsiteCategory.Official:
							return GameWebsiteType.Official
						case IgdbWebsiteCategory.Reddit:
							return GameWebsiteType.Reddit
						case IgdbWebsiteCategory.Steam:
							return GameWebsiteType.Steam
						case IgdbWebsiteCategory.Twitter:
							return GameWebsiteType.Twitter
						default:
							return GameWebsiteType.Unknown
					}
				})(),
				url,
			}))
...
export const mapStatus = (status: IgdbStatus): GameStatus | undefined => {
	switch (status) {
		case IgdbStatus.Released:
			return GameStatus.Released
		case IgdbStatus.Alpha:
			return GameStatus.Alpha
		case IgdbStatus.Beta:
			return GameStatus.Beta
		case IgdbStatus.EarlyAccess:
			return GameStatus.EarlyAccess
		case IgdbStatus.Offline:
			return GameStatus.Offline
		case IgdbStatus.Cancelled:
			return GameStatus.Cancelled
		case IgdbStatus.Rumored:
			return GameStatus.Rumored
		case IgdbStatus.Delisted:
...
const mapMultiplayerMode = (multiplayerModes: Array<IgdbMultiplayerMode>) =>
	multiplayerModes.map(
		({
			campaigncoop: hasCampaignCoop,
			dropin: hasDropIn,
			lancoop: hasLanCoop,
			offlinecoop: hasOfflineCoop,
			offlinecoopmax,
			offlinemax,
			onlinecoop: hasOnlineCoop,
			onlinecoopmax,
			onlinemax,
			splitscreen: hasSplitScreen,
			splitscreenonline: hasOnlineSplitScreen,
			platform,
		}) => ({
			hasCampaignCoop,
			hasDropIn,
...
export const mapIgdbGame = (game: IgdbGame) => {
	const {
		...
		aggregated_rating_count: ratingCount,
		genres = [],
		summary,
	...
	return {
		...
		releaseDates:
			releaseDates.length > 0
				? releaseDates.map(({ date, platform }) => ({
						platform: {
							id: platform.id,
							name: platform.name,
		...
		similarGames:
			similarGames.length > 0
				? similarGames.map(({ id, name, cover }) => ({
						id,
						name,
						cover: cover?.image_id ? `${igdbImageUrl}/t_cover_big/${cover.image_id}.jpg` : undefined,
					}))
