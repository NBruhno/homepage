...
export const mapIgdbGame = (game: IgdbGame) => {
	...
	return {
		...
		playerPerspectives: playerPerspectives.length > 0 ? playerPerspectives : undefined,
		...
		releaseDate: releaseDate ? new Date(fromUnixTime(releaseDate)).toISOString() : undefined,
		releaseDates: releaseDates.length > 0 ? releaseDates.map(({ date, platform }) => ({
			platform,
			date: date ? new Date(fromUnixTime(date)).toISOString() : undefined,
		})) : undefined,
		...
		videos: videos.length > 0 ? videos.map(({ name, video_id: videoId }) => ({ name, videoId })) : undefined,
