...
import { errorHandler, noMatchHandler, withCaching, withTracking } from 'api/middleware'

type Options = {
	/** To properly figure out when a call hit a valid method or not, we need to specify which are being used. */
 ...
 * The cache does not need to be set on a per-resource-basis, but can be set individually. This creates a handler based
 * on `next-connect`.
 * @example
 * ```ts
