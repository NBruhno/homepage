...
import { $, spinner, chalk, argv, fs } from 'zx'

$.verbose = false

const keyNames = ['access', 'refresh', 'intermediate', 'system']

const keyPairs = await spinner('Creating key pairs...', () => Promise.all(keyNames.map(async (keyName) => {
...
console.log(chalk.green('Key pairs created and appended to .env file.'))
